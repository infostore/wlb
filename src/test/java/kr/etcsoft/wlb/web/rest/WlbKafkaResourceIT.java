...
import kr.etcsoft.wlb.config.KafkaProperties;
...
class WlbKafkaResourceIT {
    ...
    void setup() {
        ...
        WlbKafkaResource kafkaResource = new WlbKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/wlb-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/wlb-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
